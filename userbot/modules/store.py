...
async def magaza(event):
    ...
    await event.edit('**GoldenUserBot Plugin Mağazası**\n__Versiyon 1.0__\n\n`🔎 Plugin\'i axtarıram... Biraz gözlə...`')
    split = plugin.split()
    if plugin == '':
        plugin = 'Son Yüklənən'
    ...
    result = f'**GoldenUserBot Plugin Mağazası**\n__Versiyon 1.0__\n\n**🔎 Axtarış:** `{plugin}`\n**🔢 Nəticələr: __({len(plugins)})__**\n➖➖➖➖➖\n\n'
...
async def sinstall(event):
    ...
    try:
        ...
        return await event.edit(f'**GoldenUserBot Plugin Mağazası**\n__Versiyon 1.0__\n\n**⚠️ Xəta:** `Plugin xətalıdır. {e}`\n**BUNU ADMİNLƏRƏ DEYİN!**')
