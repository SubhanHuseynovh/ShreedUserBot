...
async def memeyap(event):
    ...
    if event.is_reply:
        ...
        if reply.photo:
            ...
            if os.path.exists("./GoldenSticker.png"):
                os.remove("./GoldenSticker.png")

            foto = await reply.download_media()
            im = Image.open(foto).convert("RGB")
            im.save("GoldenSticker.png", "png")
            Resim = "GoldenSticker.png"
        elif reply.sticker and reply.file.ext == ".tgs":
            sticker = await reply.download_media()
            os.system(f"lottie_convert.py --frame 0 -if lottie -of png '{sticker}' GoldenSticker.png")
            os.remove(sticker)
            Resim = "GoldenSticker.png"
        elif reply.media:
            Resim = await reply.download_media()
            Sure = os.system("ffmpeg -i '"+Resim+"' 2>&1 | grep Duration | awk '{print $2}' | tr -d , | awk -F ':' '{print ($3+$2*60+$1*3600)/2}'``")
            os.system(f"ffmpeg -i '{Resim}' -vcodec mjpeg -vframes 1 -an -f rawvideo -ss {Sure} GoldenThumb.jpg")
            os.remove(Resim)
            Resim = 'GoldenThumb.jpg'
        else:
            return await event.edit(LANG['REPLY_TO_MEME'])
            
        if os.path.exists("./goldenmeme.png"):
            os.remove("./goldenmeme.png")

        MemeYap(Resim, Text, font, Bottom, BottomText)
        await event.client.send_file(event.chat_id, "./goldenmeme.png", reply_to=reply)
        await event.delete()
        os.remove(Resim)
    else:
...
async def creation(event):
    ...
    async with event.client.conversation(chat) as conv:
        try:     
            ...
            await event.reply(f"`Hmm deyəsən` {chat} `əngəlləmisən. Xaiş əngəldən çıxar.`")
            return
