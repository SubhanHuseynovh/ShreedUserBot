...
for dil in bot.iter_messages(pchannel, filter=InputMessagesFilterDocument):
    if ((len(dil.file.name.split(".")) >= 2) and (dil.file.name.split(".")[1] == "goldenjson")):
        if path.isfile(f"./userbot/language/{dil.file.name}"):
            try:
                ...
                if path.isfile("./userbot/language/DEFAULT.goldenjson"):
                    LOGS.warn("Defolt dil faylı istifadə olunur...")
                    LANGUAGE_JSON = loads(open(f"./userbot/language/DEFAULT.goldenjson", "r").read())
                else:
...
if LANGUAGE_JSON == None:
    if path.isfile(f"./userbot/language/{LANGUAGE}.goldenjson"):
        try:
            LANGUAGE_JSON = loads(open(f"./userbot/language/{LANGUAGE}.goldenjson", "r").read())
        except JSONDecodeError:
